...
class PixooDeviceClient(config: PixooConfig, private val mapper: ObjectMapper) {
  ...
  /**
   * RValue, 0-100, red (it won’t be saved and reset when the device power off) GValue, 0-100, green
   * (it won’t be saved and reset when the device power off) BValue, 0-100, blue (it won’t be saved
   * and reset when the device power off)
   */
  ...
  /**
   * Returns: UTCTime, example=1672416000, Unix epoch timestamps in seconds LocalTime,
   * example=2022-03-14 03:40:28, time in yyyy-MM-dd HH:mm:ss format
   */
  fun getSystemTime(): Map<String, Any> = genericPostCommand(GET_SYSTEM_TIME)?.parameters ?: mapOf()

  /** Longitude, -180-180, as String Latitude, -90-90, as String */
  ...
  /**
   * Returns: Weather, “Sunny”|”Cloudy”|”Rainy”|”Rainy”|”Frog”, as String CurTemp,
   * example=33.680000, the current temperature as number MinTemp, example=31.580000, the minimum
   * temperature as number MaxTemp, example=34.670000, the maximum temperature as number Pressure,
   * example=1006, current pressure as number Humidity, example=50, current humidity as number
   * Visibility, example=10000, current visibility as number WindSpeed, example=2.54, current wind
   * speed as number in m/s
   */
  fun readWeatherInformation(): Map<String, Any> =
    genericPostCommand(GET_WEATHER_INFO)?.parameters ?: mapOf()

  /**
   * Returns: Brightness, 0-100, the system brightness RotationFlag, 0|1, 1=it will switch to
   * display faces and gifs ClockTime, ??, the time of displaying faces (will be active with
   * RotationFlag = 1) GalleryTime, ??, the time of displaying gifs (will be active with
   * RotationFlag = 1) SingleGalleyTime, ??, the time of displaying each gif PowerOnChannelId,
   * Channel id, device will display the channel when it powers on CurClockId, Face id, the
   * running’s face id GalleryShowTimeFlag, 0|1, 1=it will display time at right-top Time24Flag,
   * 0|1, 1=24 hour mode; 0=12 hour mode TemperatureMode, 0|1, 0=Celsius; 1=Fahrenheit GyrateAngle,
   * 0-3, the rotation angle 0=normal; 1=90; 2=180; 3=270 MirrorFlag, 0|1, 1=screen is mirrored
